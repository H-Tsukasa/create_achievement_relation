...
def convert_batch_data(train_data, valid_data, tokenizer):
    ...
    train_iter = DataLoader(
        train_data, batch_size=batch_size_train, shuffle=True, collate_fn=generate_batch
    )
    valid_iter = DataLoader(
        valid_data, batch_size=batch_size_valid, shuffle=True, collate_fn=generate_batch
    )
...
X_train, X_test, y_train, y_test = train_test_split(
    purpose_texts, purposes, test_size=0.2, random_state=1
)
...
for loop in range(1, epochs + 1):
    ...
    loss_train = train(
        model=model, data=train_iter, optimizer=optimizer, PAD_IDX=PAD_IDX
    )
    elapsed_time = time.time() - start_time
    loss_valid = evaluate(model=model, data=valid_iter, PAD_IDX=PAD_IDX)
    print(
        "[{}/{}] train loss: {:.4f}, valid loss: {:.4f} [{}{:.0f}s] counter: {} {}".format(
            loop,
            epochs,
            loss_train,
            loss_valid,
            str(int(math.floor(elapsed_time / 60))) + "m"
