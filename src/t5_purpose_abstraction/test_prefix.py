...
with open("./test_results/result_prefix.txt", "w") as f:
      for i, line in enumerate(purpose_texts):
        ...
        token=tokenizer(input_data,
                truncation=True,
                max_length=max_seq_length,
                padding="max_length")
        output = trained_model.generate(input_ids = torch.tensor(token['input_ids']).to("cuda").unsqueeze(0), attention_mask = torch.tensor(token['attention_mask']).to("cuda").unsqueeze(0))
