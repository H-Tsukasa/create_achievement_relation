...
from lib.EarlyStopping import *

# モデルのファインチューニング用の関数
def fine_tuning(model=None,
                ...
                patience=None,
                print_batch_log=None, device=None, batch_size=2):
    """
        Args:
            model: 対象のBERTモデル
    ...
    for epoch in range(1,num_epochs+1):
        ...
        for phase in ['train_train', 'train_eval', 'valid_eval']:
            ...
            for batch in (dataloaders_dict[phase]):
                ...
                # 10バッチ経過する毎にログを表示する
                if print_batch_log == True:
                    if batch_processed_num % 10 == 0 and batch_processed_num != 0:
                        print('Processed : ', batch_processed_num * batch_size, ' Loss : ', curr_loss)
            '''
            ...
            # ログの表示
            print('Epoch {}/{} | {:^5} | Loss:{:.4f} Acc:{:.4f} time:{:.4f}\n'.format(
            ...
            if phase == 'valid_eval':
                ...
                if early_stopping.early_stop:
                    ...
                    # 学習全体におけるログを保存
                    torch.save({'log_train_train': log_train_train,
                                'log_train_eval': log_train_eval,
                                'log_valid_eval': log_valid_eval,
                                'patience': patience,
                ...
                if save_epoch_for_interval_model is not None \
                and now_epoch % save_epoch_for_interval_model == 0:
                    torch.save({'epoch': rem_epochs,
