...
def load_and_cache_examples(data_dir=None, mode=None, max_seq_length=None,
                            tokenizer=None, pad_token_label_id=None,
                            label_names=None, text_column_name=None,
                            pair_text_column_name=None, label_column_name=None):
    ...
    all_input_segment_id = torch.tensor([f.segment_ids for f in features], dtype=torch.long)
    all_input_label = torch.tensor([f.label for f in features], dtype=torch.long)
    # 上記のデータを全て、TensorDatasetにまとめる
    dataset = TensorDataset(all_input_token_id,
                            all_input_mask_id,
                            all_input_segment_id,
                            all_input_label)
    return dataset
