...
def test_prediction(
    ...
    for batch in input_dataloader:
        ...
        """
            ...
            """
        # 入力テキストをid列からトークン列へ
        input_tokens_in_batch = [
            tokenizer.convert_ids_to_tokens(token_ids) for token_ids in batch[0]
        ]
        # [PAD]トークン削除
        input_tokens_in_batch = [remove_pad(tokens) for tokens in input_tokens_in_batch]
        # トークン列をセンテンスに変換
        input_sentences_in_batch = ["".join(tokens) for tokens in input_tokens_in_batch]
        # 「##」の除去
        input_sentences_in_batch = [
            sentence.replace("##", "") for sentence in input_sentences_in_batch
        ]
        """
            正解ラベルを保持する処理------------------------------------
            """
